...
function (Interval, _) {
   ...
   */
      ...
      /*
       * ts.ZeroFilled = function (opts) { opts = _.defaults(opts, { interval:
       * '10m', start_date: null, end_date: null, fill_style: 'minimal' });
       *  // the expected differenece between readings. this.interval = new
       * Interval(opts.interval);
       *  // will keep all values here, keyed by their time this._data = {}; //
       * For each bucket in _data, store a corresponding counter of how many
       * times it was written to. this._counters = {}; this.start_time =
       * opts.start_date && getDatesTime(opts.start_date); this.end_time =
       * opts.end_date && getDatesTime(opts.end_date); this.opts = opts; };
       * 
       */

  //non timeseries zero filled
